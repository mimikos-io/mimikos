...
func TestFieldSeed_Determinism(t *testing.T) {
	reqSeed := generator.Fingerprint("GET", "/pets", nil, nil)

	seed1 := generator.FieldSeed(reqSeed, "name")
	seed2 := generator.FieldSeed(reqSeed, "name")
...
func TestFieldSeed_FieldIsolation(t *testing.T) {
	...
	nameSeed := generator.FieldSeed(reqSeed, "name")
	emailSeed := generator.FieldSeed(reqSeed, "email")

	// Adding a third field must not change the existing two.
	nameSeedAfter := generator.FieldSeed(reqSeed, "name")
	emailSeedAfter := generator.FieldSeed(reqSeed, "email")
	_ = generator.FieldSeed(reqSeed, "newField")
...
func TestFieldSeed_PathDifferentiation(t *testing.T) {
	...
	citySeed := generator.FieldSeed(reqSeed, "address/city")
	stateSeed := generator.FieldSeed(reqSeed, "address/state")
...
func TestFieldSeed_NestedPathAmbiguity(t *testing.T) {
	...
	seed1 := generator.FieldSeed(reqSeed, "a/bc")
	seed2 := generator.FieldSeed(reqSeed, "ab/c")
...
func TestFieldSeed_ArrayIndices(t *testing.T) {
	...
	seed0 := generator.FieldSeed(reqSeed, "tags/0")
	seed1 := generator.FieldSeed(reqSeed, "tags/1")
...
func TestFieldSeed_RootField(t *testing.T) {
	...
	seed := generator.FieldSeed(reqSeed, "")
...
func TestFieldSeed_DifferentRequestSeeds(t *testing.T) {
	reqSeed1 := generator.Fingerprint("GET", "/pets", nil, nil)
	reqSeed2 := generator.Fingerprint("GET", "/users", nil, nil)

	seed1 := generator.FieldSeed(reqSeed1, "name")
	seed2 := generator.FieldSeed(reqSeed2, "name")
...
func TestFieldSeed_OrderIndependence(t *testing.T) {
	reqSeed := generator.Fingerprint("POST", "/users", nil, nil)
	...
	for _, f := range fields {
		seedsForward[f] = generator.FieldSeed(reqSeed, f)
	}

	// Compute in reverse order — must produce identical seeds.
	seedsReverse := make(map[string]int64, len(fields))
	for i := len(fields) - 1; i >= 0; i-- {
		seedsReverse[fields[i]] = generator.FieldSeed(reqSeed, fields[i])
	}
